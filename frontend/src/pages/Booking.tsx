...
export default function Booking() {
  ...
  const bookingExperiences = experiences.map((experience) => experience.title);
  ...
  const packagePrice = unitPrice ? `${selectedPackage?.price} per person` : "Price to confirm";
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const submittedTotal = parsePrice(matchingExperience.price) * submittedGuests;
    ...
    const booking = await createBooking({
      ...
      experiencePrice: matchingExperience.price,
  ...
  return (
    <main className="pt-28 sm:pt-36 lg:pt-48 pb-16 md:pb-24 px-5 sm:px-8 md:px-12 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
        ...
        <div className="lg:col-span-5 space-y-12">
          <header className="space-y-6">
            <span className="inline-block px-4 py-1.5 rounded-full bg-secondary-container text-on-secondary-container text-sm font-bold tracking-widest uppercase">Reserve Package</span>
            ...
            <form className="space-y-8" onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                <div className="space-y-2 min-w-0 overflow-hidden">
                  <label className="block text-sm font-bold text-on-surface tracking-wide px-1" htmlFor="booking-experience">
                    SELECT PACKAGE
                  </label>
                  <select
                    className="block w-full max-w-full min-w-0 h-14 bg-surface-container-high border border-outline-variant/10 rounded-xl px-4 pr-10 text-sm sm:text-base text-on-surface text-ellipsis whitespace-nowrap overflow-hidden focus:outline-none focus:bg-surface-container-lowest focus:border-primary focus:ring-2 focus:ring-primary/15 transition-colors appearance-none cursor-pointer"
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-on-surface tracking-wide px-1" htmlFor="guest-count">
                    NUMBER OF GUESTS
                  </label>
                  <div className="relative">
                    <input
                      className="w-full h-14 bg-surface-container-high border border-outline-variant/10 rounded-xl px-4 text-on-surface focus:outline-none focus:bg-surface-container-lowest focus:border-primary focus:ring-2 focus:ring-primary/15 transition-all"
                      inputMode="numeric"
                      id="guest-count"
                      max="20"
                      min="1"
              ...
              {selectedPackage && (
                <section className="bg-surface-container-high rounded-2xl p-5 sm:p-6 space-y-5">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    <div>
                      <p className="text-xs uppercase tracking-widest font-bold text-primary mb-2">{selectedPackage.tag || "Package"}</p>
                      <h2 className="font-headline text-2xl font-black text-on-surface">{selectedPackage.title}</h2>
                      <p className="text-sm text-on-surface-variant leading-relaxed mt-2">{selectedPackage.description}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-3 sm:min-w-56">
                      <div className="bg-surface rounded-xl p-4">
                        <p className="text-xs uppercase tracking-widest font-bold text-on-surface-variant">Duration</p>
                        <p className="font-headline text-xl font-black text-on-surface mt-1">{selectedPackage.duration}</p>
                      </div>
                      <div className="bg-surface rounded-xl p-4">
                        <p className="text-xs uppercase tracking-widest font-bold text-on-surface-variant">Price</p>
                        <p className="font-headline text-xl font-black text-on-surface mt-1">{priceLabel(selectedPackage.price).replace(" / person", "")}</p>
                      </div>
                    </div>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-widest font-bold text-on-surface-variant mb-3">Included</p>
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {selectedPackage.included.map((item) => (
                        <li className="flex items-start gap-2 text-sm font-bold text-on-surface" key={item}>
              ...
              <div className="space-y-2">
                <label className="block text-sm font-bold text-on-surface tracking-wide px-1" htmlFor="preferred-date">
                  PREFERRED DATE
                </label>
                <div className="relative">
                  <input ref={preferredDateRef} className="date-input w-full h-14 bg-surface-container-high border border-outline-variant/10 rounded-xl px-4 pr-12 text-on-surface focus:outline-none focus:bg-surface-container-lowest focus:border-primary focus:ring-2 focus:ring-primary/15 transition-all" id="preferred-date" min={todayString()} name="preferredDate" type="date" required />
                  <button
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    <div className="space-y-2">
                      <label className="block text-sm font-bold text-on-surface tracking-wide px-1" htmlFor="email-address">
                        EMAIL ADDRESS
                      </label>
                      <input
                        className={`w-full h-14 border-none rounded-xl px-4 text-on-surface focus:ring-0 transition-all ${
                    ...
                    <div className="space-y-2">
                        <label className="block text-sm font-bold text-on-surface tracking-wide px-1" htmlFor="phone-number">
                          PHONE NUMBER
                        </label>
                      <input
                        className={`w-full h-14 border-none rounded-xl px-4 text-on-surface focus:ring-0 transition-all ${
              ...
              <section className="bg-surface-container-high rounded-2xl p-5 space-y-5">
                <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-on-surface-variant">{packagePrice}{unitPrice ? ` x ${guestCount} guest${guestCount === 1 ? "" : "s"}` : ""}</p>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <label className={`rounded-xl p-4 border cursor-pointer ${paymentChoice === "deposit" ? "border-primary bg-white" : "border-transparent bg-surface-container-low"}`}>
                    ...
                    <span className="block text-sm text-on-surface-variant">{totalAmount ? `30% now: ${formatUsd(depositAmount)}` : "Team will confirm the payable amount"}</span>
                  </label>
                  <label className={`rounded-xl p-4 border cursor-pointer ${paymentChoice === "full" ? "border-primary bg-white" : "border-transparent bg-surface-container-low"}`}>
                    ...
                    <span className="block text-sm text-on-surface-variant">{totalAmount ? `Total due: ${formatUsd(totalAmount)}` : "Team will confirm the payable amount"}</span>
                  </label>
                </div>
              </section>
              <div className="pt-4">
      ...
      {/* Package Cards (Contextual Discovery) */}
      <section className="mt-16 md:mt-24 pt-16 md:pt-24 border-t border-outline-variant/10">
        <div className="mb-10 md:mb-16 space-y-4 text-center md:text-left">
          <h2 className="text-3xl sm:text-4xl font-bold text-on-surface">Curated Highland Packages</h2>
          <p className="text-on-surface-variant max-w-2xl">Discover the stories woven into every hill. Choose the package that speaks to your spirit.</p>
        </div>
          ...
          <Link className="bg-surface rounded-2xl overflow-hidden group shadow-sm border border-outline-variant/5 block" key={experience.id} to={`/booking?experience=${encodeURIComponent(experience.title)}#booking-form`}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-between items-center pt-2">
                <span className="text-primary font-bold">{priceLabel(experience.price)}</span>
...
function priceLabel(price: string) {
  return price === "$0" || price === "$0.00" ? "Price to confirm" : `${price} / person`;
}
